...
# and `pip install courbes` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "courbes", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n courbes -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "courbes", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "courbes", "YOUR_CONSOLE_SCRIPT"]
